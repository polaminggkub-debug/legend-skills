...
3. Copy this entire `matt` directory, including its references, to each approved global skill directory. Context Discovery is selected when the task has a knowledge gap; the GitHub Projects workflow is selected when the repository or task uses Projects. Keep repository IDs, field mappings, and commands in that repository's configuration.
...
8. If the global configuration is absent, immediately ask for:
   - agent targets;
   - default tracker: none, GitHub Issues, GitHub Projects (with repository Issues), Beads, or a named custom tracker;
...
For an update with existing defaults, preserve them unless the user requests a change. Global installation makes the workflow available across projects; changing a default tracker or migrating live tasks is a separate operation.
