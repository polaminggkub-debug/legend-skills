...
3. Copy this entire `matt` directory, including its references, to each approved global skill directory. The GitHub Projects workflow is loaded on demand in any repository that selects it; keep repository IDs, field mappings, and commands in that repository's configuration.
